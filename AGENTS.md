...
- `apps/web` (`@study-reader/web`): Vite SPA + React 19 + TanStack Router (file-based) + TanStack Query + Tailwind v4 apenas para layout. **UI = elementos nativos + classes ink (`ink-*`) + `.sr-button`** — os Web Components `e-*` da epaper-components movem os light-DOM children no upgrade, o que quebra deleções do React (`removeChild`); a lib é consumida como design tokens + CSS (`tokens/base/components.css`), nunca como elementos na árvore React. Estética papel/e-ink: tokens `--ink-*`, sem sombra, gradiente ou animação.
